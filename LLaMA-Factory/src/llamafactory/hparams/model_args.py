...
class BaseModelArguments:
    ...
    new_special_tokens_config: Optional[str] = field(
        default=None,
        metadata={"help": "Add Special tokens by reading the configuration file."},
    )
    init_special_tokens: Optional[str] = field(
        default='noise_init',
        metadata={"help": "Whether to initialize the newly added token. "
                          "choices: ['noise_init', 'semantic_init', 'semantic_init_w_noise']"},
    )
    ...
    def __post_init__(self):
        ...
        if self.new_special_tokens_config is not None:
            ...
            logger.info_rank0(f"Load special_tokens config from: {self.new_special_tokens_config}")
            self.new_special_tokens_config = OmegaConf.to_container(cfg)  # to dict

        init_special_tokens_choices = ['noise_init', 'semantic_init', 'semantic_init_w_noise']
        if self.init_special_tokens not in init_special_tokens_choices:
            logger.info_rank0(f"Unsupported method: {self.init_special_tokens}, 'noise_init' is used by default")
