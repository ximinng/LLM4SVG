...
def patch_tokenizer(tokenizer: "PreTrainedTokenizer", model_args: "ModelArguments") -> None:
    ...
    elif model_args.new_special_tokens_config is not None:
        new_added_tokens = list(model_args.new_special_tokens_config.keys())
        num_added_tokens = tokenizer.add_special_tokens(
            dict(additional_special_tokens=new_added_tokens),
            replace_additional_special_tokens=False,
        )
        logger.info_rank0("Add {} to special tokens by reading config.".format(",".join(new_added_tokens)))
        if num_added_tokens > 0 and not model_args.resize_vocab:
            model_args.resize_vocab = True
            logger.warning_rank0("New tokens have been added, changed `resize_vocab` to True.")
...
def patch_model(
    ...
    if model_args.resize_vocab:
        ...
        resize_embedding_layer(model, tokenizer, model_args.new_special_tokens_config, model_args.init_special_tokens)
