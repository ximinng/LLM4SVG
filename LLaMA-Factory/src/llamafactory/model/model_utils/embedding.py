...
def _description_based_initialization(
        ...
        add_noise: bool = False  # Control noise addition
) -> None:
    ...
    for i, desc in enumerate(descriptions):
        ...
        with torch.no_grad():
            ...
            if len(valid_token_ids) == 0:
                ...
                noise = torch.normal(mean=0, std=(1.0 / math.sqrt(embedding_dim)),
                                     size=(embedding_dim,)) if add_noise else 0
                embed_weight[-num_new_tokens + i] = avg_weight + noise
            else:
                valid_token_ids = valid_token_ids.to(device)
...
def resize_embedding_layer(
        model: "PreTrainedModel",
        tokenizer: "PreTrainedTokenizer",
        new_special_tokens_config: dict = None,
    ...
    if len(tokenizer) > current_embedding_size:
        ...
        with context_maybe_zero3:
            ...
            logger.info_rank0(f"Token embeddings: adding {num_new_tokens} new tokens "
                              f"with '{init_special_tokens}' initialization strategy")

            if init_special_tokens == 'semantic_init' and new_special_tokens_config is not None:
            ...
            elif init_special_tokens == 'semantic_init_w_noise' and new_special_tokens_config is not None:
                _description_based_initialization(
                    model.get_input_embeddings().weight.data,
                    num_new_tokens,
                    new_special_tokens_config,
            ...
            else:  # init_special_tokens == 'noise_init'
                _noisy_mean_initialization(model.get_input_embeddings().weight.data, num_new_tokens)
                if model.get_output_embeddings() is not None and not model.config.tie_word_embeddings:
                    _noisy_mean_initialization(model.get_output_embeddings().weight.data, num_new_tokens)
